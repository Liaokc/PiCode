...
**终端停靠（Terminal Dock）**：
主区下方唯一底部 dock 中的用户 shell 面板；⌘J 打开/关闭，或在与桥接面板同框时把内容切回终端。⌘J/标题栏钮打开、从桥接切回、+ 重生 shell 时输入焦点立即进入 shell（dock 模型的焦点请求序列驱动；restart 同语义），桥接可见时终端绝不抢焦点。隐藏面板不结束 shell，关闭终端标签才结束。
...
**技能卡（Skill Card）**：
Composer 输入区顶部的结构化命令卡：slash 菜单选中技能或 prompt 模板后渲染（紫罗兰魔杖图标 + 名称 + × 移除，形态对照 ZCode），参数文本跟卡后。**单槽 + 替换**——Pi 语义一条消息一个行首命令，同时最多一张卡，再选即替换；**既有文本共存**——菜单选中只剥离触发 token（`/` + 查询，止于 caret，连同其后分隔空白），先有文本整段保留为卡后参数、图片独立幸存，caret 落余文开头（票 118）；卡在场时输入 `/` 仍开菜单；发送时重组 `/skill:name args` / `/name args`，发送语义与裸文本时代逐字节一致（纯渲染层，零契约增量）；草稿随卡保留。手打 `/skill:` 前缀可剥匹配菜单技能行。New Task 与会话内两处 composer 共组件同规则。
...
侧栏筛选钮打开的视图/排序下拉：**视图（View）**二选一——By project（按项目分组）或 Timeline（时间线，全部会话平铺、置顶区保留顶部）；**排序方式（Sort by）**三选一——Updated（文件 mtime）、Created（文件 birthtime，平台缺失时降级为会话头时间戳）、Manual（票 84 的手动拖序）；三排序下死 cwd 组恒沉底于所有活组（组排序活性桶，票 123——文件夹存在性优先于一切排序键，死组间次序仍按各自排序键）。排序作用于两种视图；当前选择持久化于本地偏好。文本筛选行已退役（⌘K 覆盖搜索）。
_Avoid_: 筛选输入框（已退役）；分组/项目切换钮（被下拉取代）

**手动排序（Manual Sort）**：
侧栏拖拽重排（票 84）：Projects 视图内**组内会话拖序**（行本体拖拽）与**组间拖序**（组行 ⋮⋮ grip 为真拖拽句柄；Projects 分区行无 grip——分区不重排）。首次拖拽自动切入 Manual 排序（快照当下渲染序 + 落位，视图零跳动），下拉可勾选/切回；切回 Updated/Created 即自动排序生效（手动序保留不生效），再拖即重新进入。手动序（组内行序 + 组序）持久化于本地偏好，重启保留；Timeline 视图与置顶区不参与拖拽（Manual 下 Timeline 平铺手动组序）；灰行照常可拖；**死 cwd 组恒沉底且 Manual 对其不生效**（票 123：死组组行 grip 不可拖、永不进手动序数组——沉底是活性投影不落库，目录复现即回自然排序位；活组拖序与死组内行序不受影响）；**跨项目移动会话不做**（会话项目身份 = 文件头 cwd——改它 = 写 Pi 会话文件 = 红线）；拖拽只写本地偏好，会话文件零改动。放置指示 = 目标边缘的细 accent 线。
...
**折叠聚合（Collapse all / Expand all）**：
Projects 分区行右侧常驻双钮（票 95，分区行 grip 腾出的落点）：一键折叠/展开全部**可见**组，各组各自保留形状记忆（单组语义票 39 原样放大——折叠前步进位置展开即复后），与手动单组折叠同栈混用安全（expand-all 清手动折叠、单组 toggle 可在聚合后单独复开）；Timeline 无分组不渲染此行；置顶区不受影响；无分组时 no-op；无持久化（重启回默认，与票 39 同口径）。
_Avoid_: 隐藏分组（聚合只折叠可见组，隐藏项目形状不动）；持久化记忆（形状记忆仍会话期内存级）

**占位会话卡（Pending Card）**：
create 派发瞬间乐观注入侧栏的会话占位（票 106）：以已知事实渲染（cwd 分组、首条消息投影标题——与索引扫描器同源投影，对账零文字跳变），经既有分组/排序管线并入（registry 合并）；`session_created` 盖真实 id（只记账不改显示），**会话索引到盘才对账替换**（确认源 = 索引，轮询机制不动）；boot 失败（supervisor provisional id 域内失败，或已宣布 id 未落盘即死）移除占位 + 逐字错误 toast，无幽灵条目。诚实三律：时间槽只读 `starting…`（不伪装 recency）、状态点空槽（不做活性声明）、合成 id 永不冒充真实会话（点/钉/改名/归档/拖拽/右键全惰性，永不进手动排序）。⌘K 与 Follow 不列占位（尚不可寻址）。
...
**回底钮（Jump to Latest）**：
滚离转录底部超过吸底阈值（160px）时浮现于 Composer 上方中央的圆形 ↓ 钮（ZCode 同型：card 底 outline 圆钮，票 45）；点击平滑回底并恢复吸底；吸底态隐藏，显隐淡入淡出。配套**滚离保持**（Q12 行为变更 + 票 75 方向感知——滚轮永远赢）：流式期间用户上翻阅读保持原地——向上滚动手势（任意量，只要离底）立即解除吸底（reader-held-away 闩），置位期间内容增长绝不拽人（带内强吸不再违背「内容增长不拽人」的立法本意）；滚回底部（160px 带内）/ 自己发送（含 steer/follow-up）/ 点回底钮才恢复。吸底决策收敛为纯函数 `shouldAutoScroll`（滚动状态 × heldAway 闩 × 内容增长 × 是否自己发送，全 16 组合表驱动）；160px 阈值不再单独驱动对上翻读者的强吸，保留给回底钮显隐、闩的回底恢复臂与未-held 跟随门。**自发送闩**（票 93）：发送 pin 非单次消费，而是**到达底部才清的闩**（`nextSendLatch`/`isAtBottom`）——四路发送（idle 发送 / steer / follow-up / 排队注入——队列注入本身不改 entries，靠原手势的闩存活到投递）都靠它保证「发送后视图落底、新回合 Working 容器为输入框上最底元素」；闩在每次裁定 pass 生效（entry 晚到/多轮增长/queue 面板或 composer 收回改 viewport 均重钉），只有两种清位：视图真正到底（arrival，含手动滚到底与底部钳制）或**真实上滑手势接管**（滚轮也赢自发送 agency——发送后、echo 落地前的上滑不再被 pin 回拽）。仅 ChatView；FollowView 不做。
...
**队列卡（Queue Panel）**：
composer 上方的待注入消息列表面板（票 100 交付、票 128 重构定稿）：ZCode 构图——行 = 左拖动柄（⋮⋮）+ Steer/Follow-up 角标 + 消息文本 + Edit（铅笔）+ 垃圾桶；Steer 与 Follow-up 两段分组带角标，段内**拖动柄重排**（越上越先注入；steer 注入当前回合、follow-up 排队回合后的时机语义不变），行内 Edit（预填原文+原图）与垃圾桶（单条废弃）；无全局 Clear（垃圾桶替代）。ZCode 队列卡同构。_Avoid_：Clear 钮（已随票 128 退役）；消息队列（SDK agent 队列的 UI 镜像，非抽象队列）；立即注入（「↑ 立即」已裁决不做）。
...
**用户泡（User Bubble）**：
转录中用户消息的右对齐泡，**组合块**（票 97，spec R19/R17）：三段按存在性组合——①技能渲染（魔杖 icon + Skill + 名字，复用退役 marker 视觉；有技能时）②用户文本（剥离技能注入序言的原话；有文字时）③图片缩略图条（已发消息的图片；有图时）。skill-only 泡内只渲染技能（空灰盒消失）；技能+文字泡内两者都渲染。渲染决策收敛于 Seam-1 纯模型（`userBubbleSegments`，全组合表测试）；泡是回合头静态块，不进容器流（与 R15 无冲突）。ChatView 与 FollowView 同规则。
_Avoid_: 气泡（含义过宽）；空盒（skill-only 泡不是空泡——技能段即内容）；回显（echo 是契约事件，泡是渲染物）。

**泡缩略图（Bubble Thumbnails）**：
用户泡内已发消息图片的缩略图条（票 97，spec R17）：回放数据走票 79 投影，live 靠票 97 echo 增量（发送后即时可见，不等重放）；点缩略图开**图片预览浮层**（票 91 同一组件，最小接缝复用）。缩略图自身不变（52px 全分辨率 CSS 裁切），放大是开浮层看原图。Copy 语义不变（拷用户原话，不含图片）。
_Avoid_: 图片回显（live 可见性靠 echo 投影，非轮询回显）；下载（无下载语义，预览纯视图态）。

**技能泡退役（Skill Marker Retirement）**：
容器体内技能 marker 行随票 97 退役——技能故事由**用户泡**承载（容器外，live/落定常驻，折叠不再吞），容器体不再双显；容器可展开性只由工作项决定（skill-only 回合容器体空、不可展开——可展开 ⇔ 体非空）。**操作者批准的 ZCode 偏离**：ZCode 把技能当容器内 work item（bundle `chat.toolCall.skill.*`），PiCode 改由泡承载。
_Avoid_: 双显（泡与 marker 同屏渲染即违退役）；容器内技能行（已退役，不得复活）。
...
每个回合（有用户气泡）必有的一行容器（票 55）：live 显 "Working · Ns"（首个工作项出现前的静默期即常驻），落定收起为 "Worked › Ns"（时长在 **chevron 右侧**）；两态秒数均为**回合锚点条目时间戳派生**（票 108，ADR-0002 数据源）：live = now − 回合首条目 ts（用户回执，缺席降级首工作项 ts）——回合切换（视图重挂载）与折叠均从原值续算不归零（票 61 口径迁移）；落定 = 回合首→末条目 ts 跨度，**含重放回合**（条目时间戳必在，票 108 修订票 14「回放无时长」前提），本地 tick 仅作无章回退；live 期栏头与展开体底缘各驻一枚工作转环（票 103：16px 品牌强调色，头尾镜像；折叠态仅栏头一枚，落定无环），纯状态指示非转写元素；容器体在 live 期收纳整回合的时间序单流（思考/文本块/工具/审批按转写顺序，随容器流式展开，票 82），落定后收纳正文**之前**的思考/工具/审批/过程叙述（正文后的行入常显段，票 56）；技能渲染不进容器体（票 97 退役——技能故事由用户泡承载，见「技能泡退役」）；零工作项回合体为空且不可展开（无 chevron、点击无响应——可展开 ⇔ 体非空；票 97 后技能不再计入体）；常驻不消失——落定后仍在。**操作者批准的 ZCode 偏离**：ZCode 零工作回合不渲染容器（bundle 实证 `u ? … : null`），操作者裁决常驻——「正文输出也算 work 阶段，容器不允许消失」。
...
**MCP 节（MCP Section）**：
设置窗的 MCP 服务器管理面（票 89，与 Skills/Packages 同级）：pi-mcp-adapter 的多层配置面——**全局服务器 / 项目服务器双卡**（Skills 双卡同型），每行 = 有效配置合并视图（adapter 优先级序逐字段合并 + 胜出来源徽标 + 遮蔽层数徽标 + OAuth/Disabled 徽标）；启停写项目 Pi 覆盖层 `.pi/mcp.json` 的 disabled 旗标（adapter `/mcp enable|disable` 同语义——只写旗标、定义零复制）；增改删写 `/mcp setup` 的两个正规目标（项目 `.mcp.json` / 用户全局共享 `~/.config/mcp/mcp.json`），edit/delete 落胜出层自有文件；OAuth 授权流（server 行 Authenticate → 会话 host 桥触发 adapter 自己的 /mcp-auth → 系统浏览器 → localhost 回调自动完成；**手动粘贴 callback URL 兑底**；凭据全程只在 adapter/系统钥匙串——PiCode 零凭据读写）；每层打开配置文件入口；外部 host 工具配置（Cursor/Claude 等）= 只读兼容发现、绝不写，`~/.agents` 跨工具共享文件胜出 = 只读拒写。**连接状态投影**（票 96）：每行增聚焦会话 adapter 的**活状态徽标**——connected / cached / failed / needs-auth / not-connected（disabled 行保持配置 Disabled 徽标，不重复出运行时徽标）+ tool 计数芯片；host inline extension 订阅 adapter 的版本化状态快照事件（pi.events 进程内总线）、验证后以 `mcp_status` 契约事件转发（**additive 报备入 host-contract smoke**），**纯接收零命令**——查看状态绝不触发懒启动连接/授权（查看零副作用，数据源如实）；投影是纯模型：六态映射 + 第七态「无数据」诚实降级——无聚焦会话、adapter 未上报或快照未含该 server 时状态行如实说明或行上无徽标，绝不臆造状态；快照按会话 last-wins，空快照随会话关停。
_Avoid_: 插件（ZCode 市场语义）；服务器管理（含义过宽）；状态徽标（单用含义过宽——运行时连接状态称活状态徽标，配置派生的来源/层数/OAuth/Disabled 徽标同节并存）。
