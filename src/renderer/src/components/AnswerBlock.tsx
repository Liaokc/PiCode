 ...
 * text that stays visible around the fold, action row attached. SETTLED-STATE
 * ONLY (ticket 82): a live turn renders no answer block at all — its text
 * blocks stream inline inside the container's chronological single stream —
 * and the answer appears below the container in one move at settle. The
 * AFTER-ANSWER SEGMENT (常显段, ticket 56) renders below it: every row that
 * followed the answer — tools, thinking, approvals — always visible, in
 * transcript order (ZCode assistantFollowingRows shape; revises ticket 53's
 * tools-only Q11a cut). Earlier text is interim narration inside the fold
