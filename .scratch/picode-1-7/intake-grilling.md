...
### R30 Worked 时长显示（含重放回合）—— 交付行为修订（Round 12 免问定稿）
- **痛点**：agent 工作结束后不显示该回合工作了多久；要求新增时长标志，放 Worked 右边箭头的右侧。
...
- **定稿**：移除该守卫（TUI parity）；改名成功照旧 session_renamed + 索引刷新；smoke 加运行中改名回归。

### R27 终端开启即聚焦 —— 缺陷（Round 10 免问定稿）
- **痛点**：⌘J / 标题栏终端钮打开终端后，输入焦点不进终端，要再点一下。
- **根因**：dock 开合路径（toggle-terminal-panel）无聚焦调用——TerminalDock 只有面板点击才 focus xterm。
- **定稿**：dock 变可见时立即聚焦 userTerm（挂载时序 rAF/effect 票内裁量）；桥接同框切回终端同样聚焦；焦点只在终端面板可见时给（桥接面板不抢）。

### R28 新会话卡片秒出 —— 缺陷（Round 10 免问定稿）
- **痛点**：New Task 在新文件夹创建会话，侧栏分组与卡片出现非常慢；ZCode 秒出。
- **根因**：现链 = host 冷启动（spawn + SDK + pi-subagents/pi-mcp-adapter 扩展加载）→ session_created → 文件落盘 → 索引 2 秒轮询（index-service tick）→ onIndexChanged → 刷新——层层叠加。
- **定稿**：**乐观卡片**——create 派发时 renderer 立即以已知 cwd 注入占位会话卡/分组（registry 合并，现有排序/分组语义生效），`session_created` 到达后用真实 summary 对账替换；boot 失败 = 移除占位 + toast 如实。占位期间卡片状态如实（无假数据——不显 token/时间等未知量）。索引轮询不动。

### R29 Working 计时跨切换清零 —— 缺陷（Round 11 免问定稿）
- **痛点**：两个置顶会话 A/B，A 运行中（Working 计时中）切到 B 再切回 A，计时清零从 1s 重数；操作者问及非置顶与四种切换组合。
- **根因**：`useElapsedSeconds` = 纯 tick 计数器（useState(0)+interval），无回合锚点时间戳——容器跨折叠不卸载（票 55）所以折叠安全，但 **ADR-0006 切换 = 后台不渲染 + 切回重挂载**，容器卸载即归零。**所有切换路径通病**（置顶只是首先撞见的场景）。票 61 的 Thinking 行同坑已用对修法：`useElapsedClock` 从条目级开始时间戳派生。
- **定稿**：Working 计时照票 61 口径迁移——从回合锚点时间戳（用户消息/首工作项条目时间戳）派生 `now - startedAt`，重挂载不丢；落定冻结语义保持（settle 时刻 - startedAt 自然成立）；重放回合无时长降级照旧（票 14）；四种切换组合全被通用修复覆盖；FollowView 同规。
