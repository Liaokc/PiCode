...
三十五项需求（R1–R25）加十条 Round 10–17 增补（R26–R35），全部对齐实证参照（ZCode 实拍帧/bundle 键表 / Pi 包文档 / 会话记录形态）：
...
### R35 pi-mcp-adapter 2.35.0 适配

124. As an operator running the updated pi-mcp-adapter, I want PiCode's MCP surfaces verified against 2.35.0, so that the management section, status projection and OAuth flow keep working.
125. As an operator, I want any drift found during verification fixed and recorded, so that the integration tracks the adapter's current contract.
...
### R32 pi-subagents 0.70.0 适配

121. As an operator running the updated pi-subagents, I want PiCode's subagent surfaces verified against 0.70.0, so that the fleet directory, conversation tabs and stop control keep working.
122. As an operator, I want any drift found during verification fixed and recorded, so that the integration tracks the extension's current contract.

### R34 应用图标白边修复

123. As an operator, I want the Dock icon's corners transparent (only the black squircle's smooth arcs), so that no white edges show against any background.
124. As an operator, I want the π glyph and orange cursor untouched by the fix, so that only the padding corners change.

### R33 pi agent 0.86.0 升级适配

123. As an operator whose TUI is on pi 0.86.0, I want PiCode's embedded SDK upgraded to match, so that both faces speak the same session format.
124. As an operator opening a TUI-0.86-written session in PiCode, I want new entry types handled gracefully, so that seamless handoff holds across versions.
...
- 2026-09-21 (R35 增补，Round 17)：pi-mcp-adapter 2.34.0→2.35.0 适配验证（核心消费面无 breaking 实证；89/96/110 消费面重验、漂移即修）。定稿见 R35；记录见 `intake-grilling.md` Round 17。
- 2026-09-21 (R34 增补，Round 16)：应用图标白边（票 102 交付物缺陷；qlmanage 白垫底实锤 + flood-fill 修法 intake 试算验证通过）。定稿见 R34；记录见 `intake-grilling.md` Round 16。
