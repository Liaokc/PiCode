...
- 2026-09-17 (intake 立项回执)：4e 预存缺陷已立项 **109**（`.scratch/picode-1-7/issues/109-transcript-4e-fixture.md`），根因 = t69 验证（09-15）早于 76 合入（09-16 14:31）、rebase 跨语义变更未重跑 transcript；**bisect 不适用**（69 合并起每个 checkout 同型失败）。合并会话取证已由 intake 抽查复核（fixture/时间线/复现链/帧时间戳四项通过）。
