...
- [ ] Seam-1：锚定位置差纯函数（切换前后 header offset → scrollTop 校正量；吸底态特例分支）表驱动
- [ ] electron smoke：中段折叠/展开栏头视口位置不变（像素级断言）；吸底态切换保持贴底
- [ ] FollowView 同规则；票 55/56 容器语义（零工作项 inert、live 流式展开、落定自动折叠）零回归
