...
- [ ] electron smoke：运行中子代理点停止 → 确认框 → 取消不停止 / 确认后真停（状态流转 stopping→stopped 上屏）；前台子代理停止 = abort 语义断言
- [ ] 面板收起态：运行计数徽标正确出现/消失（零运行无徽标）；点击带徽标的开合钮 → 侧板开 + 目录 tab
- [ ] 确认框 Esc/外点 = 取消；停止不可逆语义如实（不提供「撤销停止」）
