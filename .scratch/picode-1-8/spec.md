...
- **依赖与波次提示（/to-tickets 用）**：同文件群 A（composer 群）R11→R12→R7 强串行（116→117→118），R18（119）弱邻接随后；同区段 B（菜单群）R3→R4+R5 串行（121→122）；独立可并行 R2/R8/R9+R15/R10/R16/R14/R13/R19/R1/R6/R20；R17 独立大票；**R21（134）本批最后实现**（116–133 全合并后，验证带全部修复启动 app）；**R22/R23（138/139）终验修复波**（对已合并代码的修订票，无阻塞，多模态；编号避让主 Agent 自管追加轮 135–137）。
...
- 2026-09-22 (requirements intake → /to-spec): 30 痛点八轮定稿（R1–R23 + P3 现状确认）；Q1–Q9 裁决全记录见 `intake-grilling.md`。工单编号 116 起全局连续，134 为自治空跑发现的 Round 6/7 增补（双根因子 + 本批最后实现）；主 Agent 自管追加轮 135–137（队列卡上移/subagent 专属侧栏/大脑图标，均已合）；138/139 为 v1.8.0 终验的 Round 8 增补（终验修复波，多模态，编号避让追加轮）。
...
- 2026-09-22 (终验修复轮)：v1.8.0 开发完成后操作者终验报 P29/P30（票 122/125 交付物未达 ZCode 参照），立票 138/139（R22/R23，多模态）；Q4=B 口径被终验改判（R23 取代票 125 的周格语义）。同期主 Agent 自管追加轮 135–137 已全部合并（run-log §5）。
