...
## 进度与剩余波次（实时，2026-09-01 更新）

**已合入 main（8/13，main 验证 573 tests 全绿）**：14、15、16、17、20、22、23、24。
并行实测：唯一一次语义级返工是 t17↔t20（两张都重构 App 会话装配架构，
阻塞边漏串）；其余均为 union 级追加合并。

**待验收+合并**：18（worktree `wt-18-terminal-bottom-dock` @ 328f76f，
Status ready-for-human → 操作者验收后 `bash scripts/merge-ticket.sh 18`）。

**剩余前沿（三张互不共热点文件，可并行）**：

| 票 | 碰撞面 | 阻塞 |
|---|---|---|
| **19** 分组悬停 | 侧栏悬停区 + 设置页 | 无（17、22 已合入） |
| **25** 后台审批 UX | host 通知 + 侧栏任务行角标 | 无（20 已合入） |
| **21** 分支只读 | host 查询 + 标题栏 | 无（最低优先级） |

19 与 25 都碰 Sidebar.tsx 但不同区域（分组行悬停 vs 任务行角标）——union 级。

**之后**：26（←19）→ 1.1 收官（剩余终态走查随票 22 清单）。
