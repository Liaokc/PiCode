...
- 2026-09-01 (merge session, T00): merged as **312e7dd** (`merge: t25-background-approval`, rebase + no-ff onto main)。验收口径：操作者明确「已验收」；实现会话记录药丸滞留（registry 折叠天然结果，绝不自动批准零代码路径）+ 橙角标（sidebarDotState 第四态）+ 系统通知深链 + approval_resolved ack 补发 + smoke bg_approval 场景 ALL GREEN + 视觉关卡 a1–a3 + 视觉反馈轮 1（药丸等宽分割线）。冲突处置（合并会话，均机械级）：Sidebar.tsx 并集保留 19 的分组菜单 effect + 25 的四态 dotFor；index.ts 导入并集（main 的 isolateVisualUserData 调用 + 25 的 startApprovalVisualIfEnabled 接线两者都要）；tracker 中间版冲突取终态。合并后 main 上 typecheck + vitest 643/643 全绿。
